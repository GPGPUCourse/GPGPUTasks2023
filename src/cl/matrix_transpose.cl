...
__kernel void matrix_transpose(__global const float *src, __global float *dst, uint src_size_y, uint src_size_x) {
    ...
    if (src_gx < src_size_y && src_gy < src_size_x) {
    ...
    if (dst_gx < src_size_x && dst_gy < src_size_y) {
