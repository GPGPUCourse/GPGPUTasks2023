bool comp(uint a, uint b, bool also_equal, uint shift, uint k) {
    uint mask_a = (a >> shift) & ((1 << k) - 1);
    uint mask_b = (b >> shift) & ((1 << k) - 1);
    return also_equal ? mask_a <= mask_b : mask_a < mask_b;
}

unsigned int bin_search(unsigned int sorted_len, __global const uint *data, uint value, bool also_equal, uint shift, uint k) {
