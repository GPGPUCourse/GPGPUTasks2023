...
int main() {
    ...
    OCL_SAFE_CALL(errcode_ret);
    cl_mem csBuf = clCreateBuffer(context, CL_MEM_WRITE_ONLY | CL_MEM_USE_HOST_PTR, bufSize, cs.data(), &errcode_ret);
    OCL_SAFE_CALL(errcode_ret);
    ...
    const char *kernel_sourcesPtr = kernel_sources.c_str();
    const size_t kernel_sourcesSize = kernel_sources.size();
    cl_program program = clCreateProgramWithSource(context, 1, &kernel_sourcesPtr, &kernel_sourcesSize, &errcode_ret);
    OCL_SAFE_CALL(errcode_ret);
    ...
    OCL_SAFE_CALL(clGetProgramBuildInfo(program, device, CL_PROGRAM_BUILD_LOG, log_size, &log[0], nullptr));
    if (log_size > 1) {
    ...
    {
        unsigned int i = 0;
        clSetKernelArg(kernel, i++, sizeof(asBuf), &asBuf);
        clSetKernelArg(kernel, i++, sizeof(bsBuf), &bsBuf);
        clSetKernelArg(kernel, i++, sizeof(csBuf), &csBuf);
