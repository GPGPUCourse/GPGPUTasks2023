...
int main() {
    ...
    cl_mem as_buf = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_USE_HOST_PTR, sizeof(float) * n, as.data(), &errcode);
    OCL_SAFE_CALL(errcode);
    cl_mem bs_buf = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_USE_HOST_PTR, sizeof(float) * n, bs.data(), &errcode);
    OCL_SAFE_CALL(errcode);
    cl_mem cs_buf = clCreateBuffer(context, CL_MEM_WRITE_ONLY | CL_MEM_USE_HOST_PTR, sizeof(float) * n, cs.data(), &errcode);
    OCL_SAFE_CALL(errcode);
    ...
    {
        std::ifstream file(project_root + "/src/cl/aplusb.cl");
    ...
    const char *raw_sources_ptr = kernel_sources.c_str();
    const size_t sources_len = kernel_sources.size();
    cl_program program = clCreateProgramWithSource(context, 1, &raw_sources_ptr, &sources_len, &errcode);
    ...
    //   - Дождаться завершения полунного события - см. в документации подходящий метод среди Event Objects
    {
        ...
        std::cout << "GFlops: " << std::setprecision(3) << ((double)n / (t.lapAvg() * 1e9)) << std::endl;
