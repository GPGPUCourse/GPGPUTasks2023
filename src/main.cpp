...
int main() {
    ...
    cl_command_queue commandQueue = clCreateCommandQueue(context, device, NULL, &errcode_ret);
    OCL_SAFE_CALL(errcode_ret);
    ...
    cl_mem asBuffer =
            clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_USE_HOST_PTR, n * sizeof(float), as.data(), &errcode_ret);
    OCL_SAFE_CALL(errcode_ret);
    cl_mem bsBuffer =
            clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_USE_HOST_PTR, n * sizeof(float), bs.data(), &errcode_ret);
    OCL_SAFE_CALL(errcode_ret);
    cl_mem csBuffer = clCreateBuffer(context, CL_MEM_WRITE_ONLY | CL_MEM_USE_HOST_PTR, n * sizeof(float), cs.data(),
                                     &errcode_ret);
    OCL_SAFE_CALL(errcode_ret);
    ...
    const char *kernelSrc = kernel_sources.c_str();
    const size_t kernelSrcSize = kernel_sources.size();

    cl_program program = clCreateProgramWithSource(context, 1, &kernelSrc, &kernelSrcSize, &errcode_ret);
    OCL_SAFE_CALL(errcode_ret);
    ...
    // TODO 10 Выставите все аргументы в кернеле через clSetKernelArg (as_gpu, bs_gpu, cs_gpu и число значений, убедитесь, что тип количества элементов такой же в кернеле)
    {
        ...
        OCL_SAFE_CALL(clSetKernelArg(kernel, i++, sizeof(cl_int), &n));
    }
    ...
    //   - Дождаться завершения полунного события - см. в документации подходящий метод среди Event Objects
    {
        ...
        double speed = 1e-9 * n / t.lapAvg();
        std::cout << "GFlops: " << speed << std::endl;
        ...
        // - Среднее время выполнения кернела равно t.lapAvg() секунд
        double bandwith = 3 * n * sizeof(float) / (1024 * 1024 * 1024 * t.lapAvg());
        std::cout << "VRAM bandwidth: " << bandwith << " GB/s" << std::endl;
    }
    ...
    // TODO 15 Скачайте результаты вычислений из видеопамяти (VRAM) в оперативную память (RAM) - из cs_gpu в cs (и рассчитайте скорость трансфера данных в гигабайтах в секунду)
    {
        ...
        for (unsigned int i = 0; i < 20; ++i) {
            // clEnqueueReadBuffer...
            cl_event event;
            OCL_SAFE_CALL(clEnqueueReadBuffer(commandQueue, csBuffer, CL_TRUE, 0, n * sizeof(float), cs.data(), 0,
                                              nullptr, &event));
            OCL_SAFE_CALL(clWaitForEvents(1, &event));
            OCL_SAFE_CALL(clReleaseEvent(event));
            t.nextLap();
        }
        double bandwidth = n * sizeof(float) / (1024 * 1024 * 1024 * t.lapAvg());
