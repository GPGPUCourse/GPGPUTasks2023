...
int main() {
    ...
    cl_device_id device;
    auto isGpu = [](cl_device_type deviceType) { return deviceType & (1 << 2); };
    for (auto &&potentialDevice : devices) {
        cl_device_type potentialDeviceType;
        OCL_SAFE_CALL(clGetDeviceInfo(potentialDevice, CL_DEVICE_TYPE, sizeof(cl_device_type), &potentialDeviceType,
                                      nullptr));
        device = potentialDevice;

        if (isGpu(potentialDeviceType))
            break;
    }
    ...
    cl_context ctx = clCreateContext(NULL, 1, &device, NULL, NULL, &errcode_ret);
    ...
    cl_command_queue queue = clCreateCommandQueue(
            ctx, device,
            0,// If set, the commands in the command-queue are executed out-of-order. Otherwise, commands are executed in-order.
            &errcode_ret);
    OCL_SAFE_CALL(errcode_ret);
    ...
    std::vector<const char *> srcs{
            kernel_sources.c_str(),
    };

    cl_program program = clCreateProgramWithSource(ctx, 1, srcs.data(), &length, &errcode_ret);
    ...
    OCL_SAFE_CALL(clBuildProgram(program, 1, &device, options, NULL, NULL));
    ...
    size_t buildLogSize;
    OCL_SAFE_CALL(clGetProgramBuildInfo(program, device, CL_PROGRAM_BUILD_LOG, 0, NULL, &buildLogSize));

    std::vector<char> buildLog(buildLogSize, 0);
    OCL_SAFE_CALL(clGetProgramBuildInfo(program, device, CL_PROGRAM_BUILD_LOG, buildLogSize, buildLog.data(), NULL));
    ...
    OCL_SAFE_CALL(clCreateKernelsInProgram(program, 1, &kernel, NULL));


    // TODO 10 Выставите все аргументы в кернеле через clSetKernelArg (as_gpu, bs_gpu, cs_gpu и число значений, убедитесь, что тип количества элементов такой же в кернеле)
    {
        unsigned int i = 0;
        OCL_SAFE_CALL(clSetKernelArg(kernel, i++, size, &asMem));
        OCL_SAFE_CALL(clSetKernelArg(kernel, i++, size, &bsMem));
        OCL_SAFE_CALL(clSetKernelArg(kernel, i++, size, &csMem));
        OCL_SAFE_CALL(clSetKernelArg(kernel, i++, sizeof(size), &size));
    }
