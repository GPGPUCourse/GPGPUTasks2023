...
//         OCL_SAFE_CALL(clGetPlatformInfo(platform, 239, 0, nullptr, &platformNameSize));
        ...
        // libs/clew/CL/cl.h:103
        // P.S. Быстрый переход к файлу в CLion: Ctrl+Shift+N -> cl.h (или даже с номером строки: cl.h:103) -> Enter
        // Найдите там нужный код ошибки и ее название - CL_INVALID_VALUE
        ...
        OCL_SAFE_CALL(clGetPlatformInfo(platform, CL_PLATFORM_VENDOR, platformNameSize, platformName.data(), &platformNameSize));
        ...
        for (int deviceIndex = 0; deviceIndex < devicesCount; ++deviceIndex) {
            ...
            // memory size CL_DEVICE_LOCAL_MEM_SIZE
            
            
            size_t deviceMemorySize = 0;
            OCL_SAFE_CALL(clGetDeviceInfo(device, CL_DEVICE_LOCAL_MEM_SIZE, 0, nullptr, &deviceMemorySize));
            
            cl_uint deviceMemory;
            OCL_SAFE_CALL(clGetDeviceInfo(device, CL_DEVICE_LOCAL_MEM_SIZE, deviceMemorySize, &deviceMemory, &deviceMemorySize));
            std::cout << "        Device memory size: " << deviceMemory << std::endl;
            
            // profile CL_DEVICE_PLATFORM
            ...
            OCL_SAFE_CALL(clGetDeviceInfo(device, CL_DEVICE_PROFILE, deviceProfileSize, deviceProfile.data(), &deviceProfileSize));
            std::cout << "        Device platform: " << deviceProfile.data() << std::endl;
