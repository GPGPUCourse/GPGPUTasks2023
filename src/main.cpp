...
std::pair <cl_platform_id, cl_device_id> get_platform_with_device() {
    cl_platform_id any_platform;
    cl_device_id any_device;
    ...
    for (int platformIndex = 0; platformIndex < platformsCount; ++platformIndex) {
        ...
        any_platform = platform;

        cl_uint devicesCount = 0;
        cl_int err = clGetDeviceIDs(platform, CL_DEVICE_TYPE_GPU, 0, nullptr, &devicesCount);
    ...
    return {any_platform, any_device};
}
...
int main() {
    ...
    auto platform_with_device = get_platform_with_device();
    cl_platform_id platform = platform_with_device.first;
    cl_device_id device = platform_with_device.second;
    ...
    cl_mem as_gpu = clCreateBuffer(context, flags, sizeof(float)*n, as.data(), &errcode);
    OCL_SAFE_CALL(errcode);

    cl_mem bs_gpu = clCreateBuffer(context, flags, sizeof(float)*n, bs.data(), &errcode);
    OCL_SAFE_CALL(errcode);
    
    flags = {CL_MEM_USE_HOST_PTR | CL_MEM_WRITE_ONLY};
    cl_mem cs_gpu = clCreateBuffer(context, flags, sizeof(float)*n, cs.data(), &errcode);
    OCL_SAFE_CALL(errcode);
    ...
    std::vector <const char*> strings{kernel_sources.data()};
    std::vector <std::size_t> lengths{kernel_sources.length()};
    cl_program program = clCreateProgramWithSource(context, 1, strings.data(), lengths.data(), &errcode);
    OCL_SAFE_CALL(errcode);
    ...
    //   - Дождаться завершения полунного события - см. в документации подходящий метод среди Event Objects
    {
        ...
        std::cout << "GFlops: " << 1.0 * n / (t.lapAvg() * 1e9) << std::endl;
        ...
        // - Среднее время выполнения кернела равно t.lapAvg() секунд
        std::cout << "VRAM bandwidth: " << 1.0*3*n*sizeof(float) / (t.lapAvg() * 1024*1024*1024)  << " GB/s" << std::endl;
    }
    ...
    // TODO 15 Скачайте результаты вычислений из видеопамяти (VRAM) в оперативную память (RAM) - из cs_gpu в cs (и рассчитайте скорость трансфера данных в гигабайтах в секунду)
    {
        timer t;
        for (unsigned int i = 0; i < 20; ++i) {
            OCL_SAFE_CALL(clEnqueueReadBuffer(command_queue, cs_gpu, CL_FALSE, 0, n * sizeof(float), cs.data(), 0, nullptr, nullptr));
            t.nextLap();
        }
