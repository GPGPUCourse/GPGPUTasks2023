...
int main() {
    ...
    auto getDevice = [](cl_device_type type, cl_device_id& sDevice) {
    ...
    cl_command_queue commandQueue = clCreateCommandQueue(context, device, cl_command_queue_properties(0), &errorcode_ret);
    if (errorcode_ret != 0) exit(errorcode_ret);
    ...
    // TODO 15 Скачайте результаты вычислений из видеопамяти (VRAM) в оперативную память (RAM) - из cs_gpu в cs (и рассчитайте скорость трансфера данных в гигабайтах в секунду)
    {
        ...
        for (unsigned int i = 0; i < 20; ++i) {
            // clEnqueueReadBuffer...
            cl_event event;
            OCL_SAFE_CALL(clEnqueueReadBuffer(commandQueue, arrayC, CL_TRUE, 0, n * sizeof(float), cs.data(), 0, nullptr, &event));
            OCL_SAFE_CALL(clWaitForEvents(1, &event));
            t.nextLap();
        }
    ...
    OCL_SAFE_CALL(clReleaseMemObject(arrayA));
