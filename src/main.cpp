...
int main() {
    ...
    for (int platformIndex = 0; platformIndex < platformsCount; ++platformIndex) {
        ...
        for (int deviceIndex = 0; deviceIndex < devicesCount; ++deviceIndex) {
            ...
            default:
                std::cout << "non-CPU/GPU";
                break;
            }
            std::cout << std::endl;

            cl_ulong deviceMemory;
            OCL_SAFE_CALL(clGetDeviceInfo(device, CL_DEVICE_GLOBAL_MEM_SIZE, sizeof(cl_ulong), &deviceMemory, NULL));
            std::cout << "        Device memory size: " << (deviceMemory >> 20) << " MB" << std::endl;
            ...
            OCL_SAFE_CALL(clGetDeviceInfo(device, CL_DEVICE_PROFILE, deviceProfileSize, deviceProfile.data(), NULL));
