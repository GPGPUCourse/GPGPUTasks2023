...
// на сервер лучше коммитить самую простую конфигурацию. Замеры по времени получатся нерелевантные, но зато быстрее отработает CI
// локально интересны замеры на самой сложной версии, которую получится дождаться
...
TEST(SpreadBits, Simple) {
    EXPECT_EQ(0b1010101, spreadBits(0b1111));
    EXPECT_EQ(0b1000101, spreadBits(0b1011));
    EXPECT_EQ(0b01010101010101010101010101010101, spreadBits(0b1111111111111111));
}
...
constexpr int NBITS = NBITS_PER_DIM /*x dimension*/ + NBITS_PER_DIM /*y dimension*/ + 32 /*index augmentation*/;

static_assert(NBITS == 64);

/// Convert xy coordinate to a 32 bit morton/z order code +
/// 32 bit index augmentation for distinguishing between duplicates
morton_t zOrder(const Point &coord, int i) {
    assert(0 <= coord.x && coord.x < (1 << NBITS_PER_DIM));
    assert(0 <= coord.y && coord.y < (1 << NBITS_PER_DIM));

    morton_t morton_code = spreadBits(coord.x) | (spreadBits(coord.y) << 1);
...
/// Callback, which by object index returns (px, py, mass).
...
void calculateForce(float x0, float y0, float m0, const std::vector<Node> &nodes, float &force_x, float &force_y) {
    ...
    while (!stack.empty()) {
        ...
        if (node.isLeaf())
            // We don't calculate force for the object itself.
            continue;
        ...
        for (int i_child : {node.child_left, node.child_right}) {
            ...
            // С точки зрения ббоксов заходить в ребенка,
            // ббокс которого не пересекаем, не нужно
            // (из-за того, что в листьях у нас точки и они не высовываются за свой регион пространства)
            // Но, с точки зрения физики, замена гравитационного влияния всех точек в регионе
            // на взаимодействие с суммарной массой в центре масс -
            // это точное решение только в однородном поле (например, на поверхности земли)
            //
            // У нас поле неоднородное, и такая замена - лишь приближение.
            // Чтобы оно было достаточно точным, будем спускаться внутрь ноды,
            // пока она не станет похожа на точечное тело
            // (маленький размер ее ббокса относительно нашего расстояния до центра масс ноды)
            ...
            stack.push_back(i_child);
            assert(stack.size() < 2 * NBITS_PER_DIM);
        }
    }
}
...
/// CPU LBVH implementation of N-Body problem.
void nbody_cpu_lbvh(DeltaState &delta_state, State &initial_state, int N, int NT,
    ...
    const int tree_size = LBVHSize(N); ///< Size of the BVH.
    std::vector<Node> nodes(tree_size);///< Nodes of BVH.
    std::vector<morton_t> codes(N);    ///< Morton codes of bodies.
    ...
    for (int t = 0; t < NT; ++t) {
        ...
        // Calculate Morton codes for all bodies.
#pragma omp parallel for
        ...
        // The codes are augumented with indices,
        // so we can extract them later.
...
#pragma omp parallel for
        ...
        buildBBoxes(nodes, buffer, N, /* use_omp = */ false
                    /*omp here can cause stuttering on start of simulation*/);

#pragma omp parallel for
        for (int i = 0; i < N; ++i) {
            calculateForce(/*x0=*/pxs[i], /*y0=*/pys[i], /*m0=*/mxs[i], nodes, dvx[i], dvy[i]);
        }
...
#pragma omp parallel for
        for (int i = 0; i < N; ++i) {
            ...
            for (int j = 0; j < N; ++j) {
                ...
                float y1 = pys[j];
                float m1 = mxs[j];
                applyToAcceleration(x0, y0, m0, x1, y1, m1, dvx[i], dvy[i]);
            }
        }
...
/// GPU LBVH implementation of N-Body problem.
...
void buildLBVHRecursive(std::vector<Node> &nodes, const std::vector<morton_t> &codes, const std::vector<Point> &points,
    ...
    for (int i_bit = bit_index; i_bit >= 0; --i_bit) {
        int split = findSplit(codes, i_begin, i_end - 1, i_bit);
        if (split < 0)
            continue;
...
void initLBVHNode(std::vector<Node> &nodes, int i_node, const std::vector<morton_t> &codes,
    ...
    // если мы лист, то просто инициализируем минус единицами (нет детей),
    // иначе ищем своб зону ответственности и запускаем на ней findSplit
    // можно заполнить пропуски в виде тудушек,
    // можно реализовать с чистого листа самостоятельно, если так проще
    ...
    int i_begin = 0, i_end = N - 1;
    // если рассматриваем не корень,
    // то нужно найти зону ответственности ноды
    if (i_node > 0) {
        ...
        if (matchedBitsWithPrev < matchedBitsWithNext) {
            ...
            assert(matchedBitsWithPrev > matchedBitsWithNext);
    ...
    int bit_index = NBITS - 1 - __builtin_clzll(codes[i_begin] ^ codes[i_end]);
    assert(i_begin < i_end);

    int split = findSplit(codes, i_begin, i_end, bit_index);
    assert(split != -1);
    assert(i_begin < split && split <= i_end);
...
void buildBBoxes(std::vector<Node> &nodes, std::vector<int> &flags, int N, bool use_omp) {
    ...
    // NBITS раз проходимся по всему дереву и
    // инициализируем только те ноды,
    // у которых проинициализированы ббоксы обоих детей
    // Не самый оптимальный вариант (O(NlogN) вместо O(N)),
    // зато легко переложить на GPU!
    for (int level = 0; level < NBITS; ++level) {
        // знаем, что листья располагаются после N-1 внутренних нод дерева,
        // а для них уже ббоксы посчитаны -> можно пропустить
        // не сработает для рекурсивно построенного дерева,
        // там такого порядка не вводили

        // чтобы не было гонки в многопоточном режиме (и, по аналогии, потом на видеокарте),
        // в первом проходе отметим ноды,
        // которые нужно обновить,
        // и только на втором проходе обновим
...
#pragma omp parallel for if (use_omp) reduction(+ : n_updated)
        for (int i_node = 0; i_node < N - 1; ++i_node) {
            ...
            ++n_updated;
            Node &node = nodes[i_node];
            Node &left = nodes[node.child_left];
...
void checkTreesEqual(const std::vector<Node> &nodes_recursive, const std::vector<Node> &nodes,
    ...
    ASSERT_EQ(root_recursive.bbox, root.bbox);
    ASSERT_FLOAT_EQ(root_recursive.mass, root.mass);
    ASSERT_FLOAT_EQ(root_recursive.cmsx, root.cmsx);
    ASSERT_FLOAT_EQ(root_recursive.cmsy, root.cmsy);
    ASSERT_EQ(root_recursive.hasLeftChild(), root.hasLeftChild());
    ASSERT_EQ(root_recursive.hasRightChild(), root.hasRightChild());
...
TEST(LBVH, CPU) {
    ...
    auto interactive_callback = [&]() -> void {
        ...
        // check unique
        for (int i = 1; i < N; ++i) {
            ASSERT_NE(codes[i - 1], codes[i]);
        }

        std::vector<Node> nodes;

        ASSERT_NO_THROW(buildLBVH(nodes, codes, points));
        ASSERT_NO_THROW(checkLBVHInvariants(nodes, N));

        std::vector<int> buffer;
        ASSERT_NO_THROW(buildBBoxes(nodes, buffer, N));
