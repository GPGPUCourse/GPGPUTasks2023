...
void bresenham(std::vector<Point> &line_points, const Point &from, const Point &to) {
    ...
    for (int x = (int) x1; x < max_x; x++) {
        ...
        if (line_points.back() != point) { line_points.push_back(point); }
    ...
    if (line_points.back() != to) { line_points.push_back(to); }

    if (line_points.size() > 2 && flip) { std::reverse(++line_points.begin(), --line_points.end()); }
}
...
struct BBox {
    ...
    explicit BBox(const Point &point) : BBox() { grow(point); }
...
Point makePoint(float x, float y) {
    if (x < 0.f || y < 0.f) { throw std::runtime_error("0432959435934534"); }
    return Point{int(x + 0.5f), int(y + 0.5f)};
}

void initLBVHNode(std::vector<Node> &nodes,
                  int i_node,
    ...
    while (stack_size) {
        ...
        // если запрос содержится и а левом и в правом ребенке - то они в одном пикселе
        {
            ...
            if (left.bbox.contains(x0, y0) && right.bbox.contains(x0, y0)) {
                if (left.bbox != right.bbox) { throw std::runtime_error("42357987645432456547"); }
                if (left.bbox != BBox(x0, y0)) { throw std::runtime_error("5446456456435656"); }
                continue;
            }
        }
        ...
        for (int i_child : {node.child_left, node.child_right}) {
            ...
            if (!child.bbox.contains(x0, y0) && barnesHutCondition(x0, y0, child)) {
                ...
                float dr2 = std::max(100.0f, dx * dx + dy * dy);
...
void nbody_cpu_lbvh(DeltaState &delta_state,
                    State &initial_state,
                    int N,
                    int NT,
...
#pragma omp parallel for
        ...
        if (interactive_callback) { (*interactive_callback)(pxs, pys, nodes); }
...
// in: initial conditions, out: 2D array for integration
void nbody_cpu(DeltaState &delta_state,
               State &initial_state,
               int N,
               int NT,
               const interactive_callback_t *interactive_callback = nullptr) {
...
#pragma omp parallel for
        ...
        if (interactive_callback) { (*interactive_callback)(pxs, pys, {}); }
...
void nbody_gpu_lbvh(DeltaState &delta_state,
                    State &initial_state,
                    int N,
                    int NT,
                    const interactive_callback_t *interactive_callback = nullptr) {
...
void nbody_gpu(DeltaState &delta_state,
               State &initial_state,
               int N,
               int NT,
               const interactive_callback_t *interactive_callback = nullptr) {
...
State makeRandomState(int N, int minx, int maxx, int miny, int maxy) {
    ...
    for (int i = 0; i < N; ++i) {
        ...
        result.vxs[i] = std::rand() % 3 - 1;
        result.vys[i] = std::rand() % 3 - 1;
...
void findRegion(int *i_begin, int *i_end, int *bit_index, const std::vector<morton_t> &codes, int i_node) {
    int N = codes.size();
    if (i_node < 1 || i_node > N - 2) { throw std::runtime_error("842384298293482"); }
    ...
    if (dir == 0) { throw std::runtime_error("8923482374983"); }
...
void initLBVHNode(std::vector<Node> &nodes,
    ...
    if (i_node) { findRegion(&i_begin, &i_end, &bit_index, codes, i_node); }

    bool found = false;
    for (int i_bit = bit_index; i_bit >= 0; --i_bit) {
        int split = findSplit(codes, i_begin, i_end, i_bit);
        if (split < 0) continue;
        if (split < 1) { throw std::runtime_error("043204230042342"); }
        if (split - i_begin < 1) { throw std::runtime_error("043204230042342"); }
        if (i_end - split < 1) { throw std::runtime_error("043204230042342"); }
    ...
    if (!found) { throw std::runtime_error("54356549645"); }
}
...
void growNode(Node &root, std::vector<Node> &nodes) {
    ...
    if (root.mass <= 1e-8) { throw std::runtime_error("04230420340322"); }
...
void initFlag(std::vector<int> &flags, int i_node, std::vector<Node> &nodes, int level) {
    ...
    if (node.isLeaf()) { throw std::runtime_error("9423584385834"); }

    if (!node.bbox.empty()) { return; }

    const BBox &left = nodes[node.child_left].bbox;
    const BBox &right = nodes[node.child_right].bbox;

    if (!left.empty() && !right.empty()) { flags[i_node] = level; }
}
...
#pragma omp parallel for if (use_omp) reduction(+ : n_updated)
        for (int i_node = 0; i_node < N - 1; ++i_node) {
            ...
            node.bbox.clear();
            node.bbox.grow(left.bbox);
            node.bbox.grow(right.bbox);
            node.mass = left.mass + right.mass;
            node.cmsx = (left.cmsx * left.mass + right.cmsx * right.mass) / node.mass;
            node.cmsy = (left.cmsy * left.mass + right.cmsy * right.mass) / node.mass;
            ++n_updated;
        }
    ...
    for (int y = 0; y < (int) canvas.height; ++y) {
        for (int x = 0; x < (int) canvas.width; ++x) {
            ...
            while (true) {
                ...
                if (!node.bbox.contains(point)) { break; }
                ...
                if (contains_left && contains_right) {
                    if (left.bbox != right.bbox) { throw std::runtime_error("0320423949293423"); }
                    if (left.bbox != BBox(point)) { throw std::runtime_error("867932433412341"); }
                    // left and right children are at the same pixel -> no need to go down further
                    break;
                }

                if (!contains_left && !contains_right) { break; }
...
void checkLBVHInvariants(const std::vector<Node> &nodes, int N) {
    ...
    for (const Node &node : nodes) {
        if (node.hasLeftChild() ^ node.hasRightChild()) { throw std::runtime_error("9873208597205982"); }
    }
    ...
    if (!stack.empty()) { throw std::runtime_error("94959345934534"); }
    // все ноды достигли по разу
    for (int v : used) {
        if (v != 1) { throw std::runtime_error("432543534645654"); }
    }
}
...
void nbody(bool interactive, bool evaluate_precision, int nbody_impl_index) {
    ...
    if (ENABLE_GUI) { window = std::make_shared<images::ImageWindow>("nbody"); }
    ...
    int i_frame = 0;
    timer tm_framerate;
    interactive_callback_t interactive_callback =
            [&](const std::vector<float> &pxs, const std::vector<float> &pys, const std::vector<Node> &nodes) -> void {
        canvas.fill(zero);

        if (nodes.size()) { drawLBVH(canvas, nodes, initial_state.coord_shift - canvas_size / 2); }

#pragma omp parallel for
        for (int i = 0; i < N; ++i) {
            float x = state.pxs[i] + canvas.width / 2.0f - initial_state.coord_shift;
            float y = state.pys[i] + canvas.height / 2.0f - initial_state.coord_shift;
...
void checkTreesEqual(const std::vector<Node> &nodes_recursive,
...
TEST(LBVH, CPU) {
    ...
    std::shared_ptr<images::ImageWindow> window;
    if (ENABLE_GUI) { window = std::make_shared<images::ImageWindow>("lbvh_naive"); }
