...
// имеет смысл отключать при оффлайн симуляции больших N, но в итоговом решении
// стоит оставить
...
// TODO на сервер лучше коммитить самую простую конфигурацию. Замеры по времени
// получатся нерелевантные, но зато быстрее отработает CI
// TODO локально интересны замеры на самой сложной версии, которую получится
// дождаться
#define NBODY_INITIAL_STATE_COMPLEXITY 0
// #define NBODY_INITIAL_STATE_COMPLEXITY 1
// #define NBODY_INITIAL_STATE_COMPLEXITY 2

// использовать lbvh для построения начального состояния. Нужно на очень больших
// N (>1000000)
...
// Convert xy coordinate to a 32 bit morton/z order code + 32 bit index
// augmentation for distinguishing between duplicates
...
// из аугментированного мортоновского кода можно извлечь индекс в изначальном
// массиве
...
bool barnesHutCondition(float x, float y, const Node &node) {
    ...
    // возвращаем true, если находимся от ноды достаточно далеко, чтобы можно было
    // ее считать примерно точечной
...
void calculateForce(float x0, float y0, float m0, const std::vector<Node> &nodes, float *force_x, float *force_y) {
    ...
    while (stack_size) {
        ...
        // если запрос содержится и в левом и в правом ребенке - то они в одном
        // пикселе
        {
        ...
        for (int i_child : {node.child_left, node.child_right}) {
            ...
            // С точки зрения ббоксов заходить в ребенка, ббокс которого не
            // пересекаем, не нужно (из-за того, что в листьях у нас точки и они не
            // высовываются за свой регион пространства)
            //   Но, с точки зрения физики, замена гравитационного влияния всех точек
            //   в регионе на взаимодействие с суммарной массой в центре масс - это
            //   точное решение только в однородном поле (например, на поверхности
            //   земли) У нас поле неоднородное, и такая замена - лишь приближение.
            //   Чтобы оно было достаточно точным, будем спускаться внутрь ноды, пока
            //   она не станет похожа на точечное тело (маленький размер ее ббокса
            //   относительно нашего расстояния до центра масс ноды)
...
int findSplit(const std::vector<morton_t> &codes, int i_begin, int i_end, int bit_index) {
    ...
    while (r - l > 1) {
        int m = (l + r) / 2;
        int bit = getBit(codes[m], bit_index);
        if (bit == 0)
            l = m;
        else
            r = m;
    ...
    // избыточно, так как на входе в функцию проверили, что ответ существует, но
    // приятно иметь sanity-check на случай если набагали
...
void findRegion(int *i_begin, int *i_end, int *bit_index, const std::vector<morton_t> &codes, int i_node) {
    ...
    // 1. найдем, какого типа мы граница: левая или правая. Идем от самого
    // старшего бита и паттерн-матчим тройки соседних битов
    //  если нашли (0, 0, 1), то мы правая граница, если нашли (0, 1, 1), то мы
    //  левая
    // dir: 1 если мы левая граница и -1 если правая
    int dir = 0;
    int i_bit = NBITS - 1;
    for (; i_bit >= 0; --i_bit) {
        bool a = codes[i_node - 1] & (1L << i_bit);
        bool b = codes[i_node] & (1L << i_bit);
        bool c = codes[i_node + 1] & (1L << i_bit);
        if (a < b) {
            dir = 1;
            break;
        }
...
void initLBVHNode(std::vector<Node> &nodes, int i_node, const std::vector<morton_t> &codes,
    ...
    // если мы лист, то просто инициализируем минус единицами (нет детей), иначе
    // ищем своб зону ответственности и запускаем на ней findSplit можно заполнить
    // пропуски в виде тудушек, можно реализовать с чистого листа самостоятельно,
    // если так проще
    ...
    // если рассматриваем не корень, то нужно найти зону ответственности ноды и
    // самый старший бит, с которого надо начинать поиск разреза
    if (i_node) {
    ...
    for (int i_bit = bit_index; i_bit >= 0; --i_bit) {
        ...
        // проинициализировать nodes[i_node].child_left, nodes[i_node].child_right
        // на основе i_begin, i_end, split
...
void buildLBVH(std::vector<Node> &nodes, const std::vector<morton_t> &codes, const std::vector<Point> &points) {
    ...
    // можно раскомментировать и будет работать, но для дебага удобнее оставить
    // однопоточную версию
#pragma omp parallel for
    for (int i_node = 0; i_node < tree_size; ++i_node) {
        initLBVHNode(nodes, i_node, codes, points_mass_array);
...
void buildBBoxes(std::vector<Node> &nodes, std::vector<int> &flags, int N, bool use_omp) {
    ...
    // NBITS раз проходимся по всему дереву и инициализируем только те ноды, у
    // которых проинициализированы ббоксы обоих детей
    //   не самый оптимальный вариант (O(NlogN) вместо O(N)), зато легко
    //   переложить на GPU
    for (int level = 0; level < NBITS; ++level) {
// знаем, что листья располагаются после N-1 внутренних нод дерева, а для них
// уже ббоксы посчитаны -> можно пропустить не сработает для рекурсивно
// построенного дерева, там такого порядка не вводили

// чтобы не было гонки в многопоточном режиме (и, по аналогии, потом на
// видеокарте), в первом проходе отметим ноды, которые нужно обновить, и только
// на втором проходе обновим
...
#pragma omp parallel for if (use_omp) reduction(+ : n_updated)
        for (int i_node = 0; i_node < N - 1; ++i_node) {
            // TODO если находимся на нужном уровне (нужный flag), проинициализируем
            // ббокс и центр масс ноды
        ...
        for (int x = 0; x < (int) canvas.width; ++x) {
            ...
            while (true) {
                ...
                if (contains_left && contains_right) {
                    ...
                    // left and right children are at the same pixel -> no need to go down
                    // further
                    break;
                }
...
void checkLBVHInvariants(const std::vector<Node> &nodes, int N) {
    ...
    // у каждой ноды либо нет ни одного ребенка, тогда она лист, либо есть два
    // ребенка
    ...
    // каждая нода достижима из корня, причем только один раз (только один
    // родитель)
...
TEST(LBVH, GPU) {
    ...
    for (int i = 0; i < tree_size; ++i) {
        EXPECT_EQ(nodes[i], nodes_cpu[i]) << "Not equals nodes at index " << i;
    }
