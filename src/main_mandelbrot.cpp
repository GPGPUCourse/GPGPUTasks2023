...
int main(int argc, char **argv)
{
    ...
    uint benchmarkingIters = 10;

    uint width = 2048;
    uint height = 2048;
    uint iterationsLimit = 256;
...
//    float sizeX = 2.0f;
...

    {
        ...
        size_t gflops = 1e9;
        std::cout << "CPU: " << timer.lapAvg() << "+-" << timer.lapStd() << " s" << std::endl;
        std::cout << "CPU: " << maxApproximateFlops / gflops / timer.lapAvg() << " GFlops" << std::endl;
    ...
    context.activate();
    {
        ...
        std::cout << "GPU: " << maxApproximateFlops / gflops / timer.lapAvg() << " GFlops" << std::endl;
