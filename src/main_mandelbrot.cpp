...
//    float sizeX = 2.0f;
    ...
    context.activate();
    {
        ...
        gpu::gpu_mem_32f result_buf;
        result_buf.resizeN(width * height);

        timer t;
        for (int i = 0; i < benchmarkingIters; ++i) {
            kernel.exec(gpu::WorkSize(128, (width * height + 128 - 1) / 128 * 128),
                        result_buf,
                        width, height,
                        centralX - sizeX / 2.0f, centralY - sizeY / 2.0f,
                        sizeX, sizeY,
        ...
        result_buf.readN(gpu_results.ptr(), width * height);
