...
int main(int argc, char **argv) {
    ...
    context.activate();
    {
        ...
        gpu_results_gpu.resizeN(gpu_results.width * gpu_results.height);
        ...
        unsigned int global_work_size = width * height;

        timer t;
        for (int i = 0; i < benchmarkingIters; ++i) {
            kernel.exec(
                gpu::WorkSize(workGroupSize, 1, width, height), 
                gpu_results_gpu,
                width,
                height,
                centralX - sizeX / 2.0f,
        ...
        gpu_results_gpu.readN(gpu_results.ptr(), gpu_results.width * gpu_results.height);
        double realIterationsFraction = 0.0;
        for (int j = 0; j < height; ++j) {
            for (int i = 0; i < width; ++i) {
    ...
    // Это бонус в виде интерактивной отрисовки, не забудьте запустить на ГПУ, чтобы посмотреть, в какой момент числа
    // итераций/точности single float перестанет хватать
