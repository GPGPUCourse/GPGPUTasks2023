...
int main(int argc, char **argv) {
...

    {
        ...
        std::cout << "CPU: " << maxApproximateFlops / (double) gflops / t.lapAvg() << " GFlops" << std::endl;
    ...
    context.activate();
    {
        ...
        kernel.compile(/* printLog = */ true);
        gpu::gpu_mem_32f results_gpu;
        results_gpu.resizeN(width * height);
        unsigned axisWorkGroupSize = 2;
        assert(width % axisWorkGroupSize == 0);
        assert(height % axisWorkGroupSize == 0);
        timer timer;
        for (int i = 0; i < benchmarkingIters; ++i) {
            kernel.exec(gpu::WorkSize(axisWorkGroupSize, axisWorkGroupSize, width, height),
                        /* results: */ results_gpu, width, height, centralX - sizeX / 2.0f, centralY - sizeY / 2.0f,
        ...
        std::cout << "GPU: " << maxApproximateFlops / (double) gflops / timer.lapAvg() << " GFlops" << std::endl;
    ...
    // renderInWindow(centralX, centralY, iterationsLimit, true);
