...
void mandelbrotCPU(float* results, unsigned int width, unsigned int height, float fromX, float fromY, float sizeX, float sizeY, unsigned int iters, bool smoothing) {
...
//    float sizeX = 2.0f;
    ...
    context.activate();
    {
        ...
        buffer.resizeN(width * height);

        timer t;
        for (int i = 0; i < benchmarkingIters; ++i) {
            t.start();
            kernel.exec(ws, buffer,
                        width, height,
                        centralX - sizeX / 2.0f, centralY - sizeY / 2.0f,
                        sizeX, sizeY,
                        iterationsLimit, 0);
        ...
        buffer.readN(gpu_results.ptr(), width * height);
