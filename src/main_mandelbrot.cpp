...
int main(int argc, char **argv) {
    ...
    context.activate();
    {
        ...
        {
            // это предел моего устройства =)
            gpu::WorkSize workSize(128, 64, width, height);
            ...
            timer t;
            for (int i = 0; i < benchmarkingIters; ++i) {
                // __global float *results, const unsigned int iters, const float fromX, const float fromY, const float sizeX, const float sizeY, const float threshold
                kernel.exec(
                    workSize,
                    result, // __global float *results
