...
int main(int argc, char **argv) {
    ...
    auto benchmarkKernel = [&](const std::string &kernelName, gpu::WorkSize workSize, size_t tileSize, size_t threadWork) -> void {
        std::string defines;
        if (tileSize != 0)
            defines += "-D TILE_SIZE=" + to_string(tileSize) + " ";
        if (threadWork != 0)
            defines += "-D THREAD_WORK=" + to_string(threadWork) + " ";
...

        {
            ...
            std::cout << kernelName << ": " << t.lapAvg() << "+-" << t.lapStd() << " s" << std::endl;
            std::cout << kernelName << ": " << gflops / t.lapAvg() << " GFlops" << std::endl;
        }
    ...
    benchmarkKernel("matrix_multiplication_naive", gpu::WorkSize(16, 16, N, M), 0, 0);
