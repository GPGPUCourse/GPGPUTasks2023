...
int main(int argc, char **argv) {
...

    {
...

        {
            ...
            for (int iter = 0; iter < benchmarkingIters; ++iter) {
                ...
                matrix_multiplication_kernel.exec(gpu::WorkSize(work_group_size, work_group_size, M, K), as_gpu, bs_gpu,
                                                  cs_gpu, M, K, N);

                t.nextLap();
            }
            std::cout << "GPU 1: " << t.lapAvg() << "+-" << t.lapStd() << " s" << std::endl;
            std::cout << "GPU 1: " << gflops / t.lapAvg() << " GFlops" << std::endl;
        }
    ...
    }
    {
...

        {
            ...
            std::cout << "GPU 2: " << gflops / t.lapAvg() << " GFlops" << std::endl;
        }
    ...
    }
    {
...

        {
            ...
            for (int iter = 0; iter < benchmarkingIters; ++iter) {
                ...
                matrix_multiplication_kernel.exec(gpu::WorkSize(work_group_size, work_group_size / work_per_thread, M,
                                                                (K + work_per_thread - 1) / work_per_thread),
                                                  as_gpu, bs_gpu, cs_gpu, M, K, N);

                t.nextLap();
            }
            std::cout << "GPU 3: " << t.lapAvg() << "+-" << t.lapStd() << " s" << std::endl;
            std::cout << "GPU 3: " << gflops / t.lapAvg() << " GFlops" << std::endl;
        }
