...

    {
        ...
        for (int iter = 0; iter < benchmarkingIters; ++iter) {
            ...
            unsigned int global_work_size_width =
                    ((K + 1) /2 + work_group_size_width - 1) / work_group_size_width * work_group_size_width;
            unsigned int global_work_size_height =
                    ((M + 1) /2 + work_group_size_height - 1) / work_group_size_height * work_group_size_height;
            ...
            matrix_transpose_kernel.exec(
                    gpu::WorkSize(work_group_size_width, work_group_size_height, global_work_size_width,
                                  global_work_size_height), as_gpu, as_t_gpu, M, K);
