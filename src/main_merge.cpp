...
int main(int argc, char **argv) {
    ...
    as_gpu.resizeN(n);
    gpu::gpu_mem_32f cache;
    cache.resizeN(n);
    {
        ocl::Kernel merge(merge_kernel, merge_kernel_length, "mergenew");
        merge.compile();
        timer t;
        for (int iter = 0; iter < benchmarkingIters; ++iter) {
            ...
            for (unsigned int block_size = 1; block_size < n; block_size *= 2) {
                merge.exec(gpu::WorkSize(workGroupSize, global_work_size), as_gpu, cache, n, block_size);
                std::swap(as_gpu, cache);
            }
            t.nextLap();
        }
